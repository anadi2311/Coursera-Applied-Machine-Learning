...
def answer_one():
    ...
    for i, degree in enumerate([1,3,6,9]):
        ...
        temp = linreg.predict(X_Predict_poly)
        result[i,:] = temp
...
def answer_two():
    ...
    for i, degree in enumerate(range(10)):
        ...
        temp1 = linreg.score(X_train_poly,y_train)
        temp2 = linreg.score(X_test_poly,y_test)
        r2_train.append(temp1)
        r2_test.append(temp2)
...
#     plt.figure(figsize = (10,5))
#     plt.plot(range(10), r2[0],label="train" )
#     plt.plot(range(10),r2[1],label="test")
...
def answer_four():
    ...
    las_r2 = lasreg.score(X_test_poly,y_test)

    # Your code here

    return lin_r2,las_r2
...
def answer_five():
    ...
    df = pd.Series(data = F_imp, index = X_train2.columns)
    top_five = df.sort_values(ascending = False).iloc[0:5].index
...
def answer_seven():
    import pandas as pd
    score = answer_six()
    df = pd.DataFrame({'training_score':score[0], 'test_score': score[1]}, index = np.logspace(-4,1,6))
    df['diff'] = df['training_score'] - df['test_score']
